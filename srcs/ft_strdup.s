...
_ft_strdup:
	push	rbx                          ; callee-saved 退避 & スタック 16B 整列
	mov		rbx, rdi                     ; s を rbx に退避

	call	_ft_strlen                   ; rax = strlen(s)
	lea		rdi, [rax + 1]               ; rdi = len + 1 (NUL 分)
	call	_malloc                      ; rax = malloc(len + 1)
	test	rax, rax
	jz		.done                        ; NULL なら errno は malloc 側で設定済

	mov		rdi, rax                     ; dst = malloc ptr
	mov		rsi, rbx                     ; src = 元の s
	call	_ft_strcpy                   ; rax = dst (= malloc ptr)
.done:
	pop		rbx
	ret
